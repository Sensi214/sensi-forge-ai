...
const app = express();
app.use(cors());
app.use(express.json({ limit: '50mb' })); // Increased limit for high-res image uploads

// 1. INKEDJOY PRODUCT SPEC DATABASE
...
app.get('/', (req, res) => {
  res.send('<body style="background:#000;color:#0f0;text-align:center;padding:50px;font-family:monospace;">' +
           '<h1>🏺 SENSI FORGE: ACTIVE</h1><p>Ready to Forge 16 InkedJoy Products.</p></body>');
});

// 3. THE FORGE ENDPOINT
app.post('/forge-merch', async (req, res) => {
    ...
    try {
        ...
        const output = await replicate.run(
          "tencentarc/instant-id-multicontrolnet:35324a7df2397e6e57dfd8f4f9d2910425f5123109c8c3ed035e769aeff9ff3c",
          {
            input: {
              ...
              prompt: `A professional apparel design for ${product.name}, featuring text "${tagline}", cinematic lighting, masterpiece, 8k resolution`,
              width: product.w,
              height: product.h,
        ...
        res.json({ success: true, imageUrl: output[0], sku: selectedSku });
    } catch (err) {
        console.error(err);
        res.status(500).json({ success: false, error: "The Forge encountered a critical failure." });
    }
});
